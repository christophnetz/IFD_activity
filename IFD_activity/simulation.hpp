...
void simulation(const Param& param_) {
  ...
  std::ofstream ofs6(param_.outdir + "_fitness.txt", std::ofstream::out);

  ofs5 << "gen\tscene\ttime\tcomp\tact\txpos\typos\tfood\tintake\n";
  ofs6 << "gen\ttime\tcomp\tact\txpos\typos\tfood\tintake\tfitness\n";
  ...
  for (int g = 0; g < param_.G; ++g) {
    ...
    //outputting the stuff for ofs6
    if (g % 100 == 0) {

        for (int i = 0; i < pop.size(); ++i) {

            ofs6 << g << "\t" << eat_t << "\t" << pop[i].comp << "\t" << pop[i].act << "\t" << pop[i].xpos << "\t"
                << pop[i].ypos << "\t" << pop[i].food << "\t" << pop[i].updateintake(landscape, presence) << "\t" 
                << max(0.0, pop[i].food - param_.cost * pop[i].act* param_.t_scenes - param_.cost_comp * pop[i].comp * param_.t_scenes)<<"\n";        

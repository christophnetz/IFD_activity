...
# list files
data_files <- list.files("x64/Release/data/", pattern = "social.csv",
                         full.names = T)
...
data_agg <- lapply(data, function(df){
  df_ <- copy(df)
  df_[, soc_cut := cut(socRound, breaks = seq(0, 1, 0.001), include.lowest = T)]
  df_ <- df_[, list(count = .N), by = c("pop_density", "newDensity", "f_cost", 
                                        "rep_id", "soc_cut", "gen", "basePred")]
})
...
plots <- Map(function(df) {
  ggplot(df[inrange(gen, 100, 15000), ])+
    ...
    geom_tile(aes(gen, soc_cut, fill = prop),
              show.legend = F)+
